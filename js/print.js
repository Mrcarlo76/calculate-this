...
(function(){
  ...
  const CONSTRUCTION_REPORTS={
    'plasterboard.html':{title:'Plasterboard Take-off',assumptions:['Board quantity includes the selected boarded faces and one or two layers per face.','Waste allowance is applied to net boarded area before the quantity is rounded up to full sheets.','Openings are not automatically deducted; layout, staggered joints and site cutting can alter final quantities.']},
    ...
    'screed.html':{title:'Screed Quantity Take-off',assumptions:['Net volume uses floor area multiplied by the entered average screed depth.','The selected allowance is added after the net volume is calculated.','Falls, uneven substrates and minimum specified depths can increase requirements.']},
    'concrete.html':{title:'Concrete Quantity Take-off',assumptions:['Net volume is length × width × depth, with depth converted from millimetres to metres.','The selected allowance is shown separately and added after the net volume calculation.','Excavation tolerance, uneven formation and supplier minimum loads should be checked.']},
    'insulation.html':{title:'Insulation Take-off',assumptions:['Net measured area is shown separately from the waste-adjusted ordering area.','Pack quantity is calculated from the entered manufacturer pack coverage and rounded up to full packs.','Cuts, framing centres and manufacturer installation requirements can change quantities.']},
    'brick-block.html':{title:'Brick & Block Take-off',assumptions:['Unit quantity uses the selected nominal units-per-square-metre rate.','Entered opening area is deducted from gross wall area before the unit quantity and waste allowance are calculated.','Piers, bonding patterns, cuts and actual unit dimensions should be allowed for separately where significant.']}
  };
